...
function excelDateToISO(dateVal) {
...
function collectFiles(dir, customerMap) {
...
function readSheetWithHeaders(filePath, titleRowsToSkip = 0) {
    const workbook = XLSX.readFile(filePath);
    const sheetName = workbook.SheetNames[0];
    const sheet = workbook.Sheets[sheetName];
    const rawData = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '' });
    if (rawData.length <= titleRowsToSkip)
        return [];
    const headers = rawData[titleRowsToSkip];
...
function parseChineseBatch2Row(row, filePath) {
...
function parseChineseBatch4Row(row, filePath) {
    const customerName = safeString(row['*客户名']);
    if (!customerName)
        return null;
...
function parseExcelFile(filePath, customerMap) {
    try {
        const workbook = XLSX.readFile(filePath);
        const sheetName = workbook.SheetNames[0];
        const sheet = workbook.Sheets[sheetName];
        ...
        const headers = firstRow.map((v) => safeString(v)).filter(Boolean);
...
function computeMetrics(customerMap) {
