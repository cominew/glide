...
import { EventCategory, EventFilter } from '../types/events';
  ...
  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
    ...
    {taskIds.length > 0 && (
      <select onChange={e => onTaskFilter?.(e.target.value || undefined)}
        style={{ fontSize: 12, padding: '4px 8px', borderRadius: 8, border: '0.5px solid var(--border)', background: 'var(--bg-elevated)', color: 'var(--text-secondary)', outline: 'none' }}>
        <option value="">All tasks</option>
        {taskIds.map(id => (
          <option key={id} value={id}>{id.slice(0, 24)}…</option>
        ))}
      </select>
    )}
  </div>
