// D:\.openclaw\app\web-dashboard\src\App.tsx
...
export default function App() {
  ...
  useEffect(() => {
  setLogs([
    { id: '1', timestamp: new Date().toLocaleTimeString(), level: 'info', message: 'System started' },
    { id: '2', timestamp: new Date().toLocaleTimeString(), level: 'warn', message: 'High memory usage' },
    { id: '3', timestamp: new Date().toLocaleTimeString(), level: 'error', message: 'Failed to fetch data' },
  ]);
}, []);
  ...
  const loadData = useCallback(async (manual = false) => {
    ...
    try {
      ...
      setCustomers(topCustomers.map((c, i) => ({ ...c, id: String(i) })));
  ...
  // --- Helper to send message from any tab ---
    ...
    <div className="flex h-screen w-full bg-[#020617] text-slate-300 font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-[#0b0f1a]/80 backdrop-blur-xl z-10 shrink-0">
          <div className="flex items-center gap-4 bg-slate-900/50 px-5 py-2.5 rounded-xl border border-slate-800 w-[450px] focus-within:border-blue-500/50 transition-all shadow-inner group">
            <Search size={18} className="text-slate-500 group-focus-within:text-blue-400" />
            <input type="text" id="global-search" autoComplete="off" placeholder={t.askPlaceholder} className="bg-transparent border-none outline-none text-sm w-full text-slate-200 placeholder:text-slate-600" />
          </div>
          <div className="flex items-center gap-6">
            <button onClick={() => loadData(true)} disabled={isRefreshing || connStatus === 'checking'} title={t.refresh} className="p-2 hover:bg-slate-800 rounded-lg text-slate-500 transition-colors disabled:opacity-40">
              <RefreshCw size={20} className={isRefreshing ? 'animate-spin' : ''} />
            </button>
            <button onClick={() => setLang(l => l === 'zh' ? 'en' : 'zh')} className="p-2 hover:bg-slate-800 rounded-lg text-slate-500 transition-colors">
              <Languages size={20} />
            </button>
            <div className="relative group p-2 rounded-lg hover:bg-slate-800 transition-colors cursor-pointer">
              ...
              <div className="absolute top-2 right-2 w-2 h-2 bg-red-600 rounded-full border-2 border-[#0b0f1a]"></div>
            </div>
            <div className="flex items-center gap-4 pl-8 border-l border-slate-800">
              ...
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-slate-700 to-slate-900 flex items-center justify-center border border-slate-700 shadow-xl">
        ...
        {/* Offline banner */}
        {connStatus === 'offline' && (
          <div className="px-4 py-2 bg-amber-950 border-b border-amber-800 flex items-center justify-between">
        ...
        <div className="flex-1 overflow-y-auto px-4 lg:px-8 py-6">
          ...
          {tab === 'ai' && (
            <AITab isOnline={isOnline} messages={messages} chatLoading={chatLoading} onSend={handleSend} onClear={clearMessages} />
          )}
          {tab === 'health' && (
            <HealthTab connStatus={connStatus} customersCount={customers.length} monthlyTrend={overviewData?.monthlyTrend || []} t={t} healthData={healthData} />
          )}
          {tab === 'settings' && (
            <SettingsTab lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} notifs={notifs} setNotifs={setNotifs} t={t} />
          )}
